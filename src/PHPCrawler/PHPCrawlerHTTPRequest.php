...
class PHPCrawlerHTTPRequest {
    ...
    public function __construct() {
        ...
        if (!class_exists("PHPCrawlerLinkFinder"))
            include_once(dirname(__FILE__) . "/PHPCrawlerLinkFinder.php");
        $this->LinkFinder = new PHPCrawlerLinkFinder();

        // Init DNS-cache
        if (!class_exists("PHPCrawlerDNSCache"))
            include_once(dirname(__FILE__) . "/PHPCrawlerDNSCache.php");
        $this->DNSCache = new PHPCrawlerDNSCache();

        // Cookie-Descriptor
        if (!class_exists("PHPCrawlerCookieDescriptor"))
            include_once(dirname(__FILE__) . "/PHPCrawlerCookieDescriptor.php");

        // ResponseHeader-class
        if (!class_exists("PHPCrawlerResponseHeader"))
            include_once(dirname(__FILE__) . "/PHPCrawlerResponseHeader.php");

        // PHPCrawlerHTTPProtocols-class
        if (!class_exists("PHPCrawlerHTTPProtocols"))
            include_once(dirname(__FILE__) . "/Enums/PHPCrawlerHTTPProtocols.php");
    }
    ...
    protected function readResponseContent($stream_to_file = false, &$error_code, &$error_string, &$document_received_completely) {
        ...
        while ($document_completed == false) {
            ...
            /**
             * Toan LE comment these lines, because in this case, we dont need to find the links.
             * We need to parse DOM HTML
             */
    ...
    protected function buildRequestHeader() {
        ...
        // Fake IP address
        $headerlines[] = "REMOTE_ADDR: 66.249.66.1\r\n";
        $headerlines[] = "HTTP_REMOTE_ADDR: 66.249.66.1\r\n";
        $headerlines[] = "X-Http-Forwarded-For: 66.249.66.1\r\n";
